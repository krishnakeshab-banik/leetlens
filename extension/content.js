...
(function () {
  ...
  function detectSubmissionSuccess() {
    // Look for success/accepted indicators
    const selectors = [
    ...
    if (allText.includes('Accepted') || allText.includes('All tests passed')) {
      return true;
    }
    return false;
  }
  ...
  function checkSubmissionStatus(slug) {
    ...
    if (isSolved !== lastSolvedState) {
      lastSolvedState = isSolved;
      const msgType = isSolved ? 'MARK_SOLVED' : 'MARK_PENDING';
      chrome.runtime.sendMessage({
        type: msgType,
        slug
      });
  ...
  // Check for submission success periodically
  setInterval(() => checkSubmissionStatus(slug), 2000);
