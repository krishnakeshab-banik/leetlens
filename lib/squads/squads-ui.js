(function () {
  ...
  function formatSprintType(type) {
    const map = { daily: 'Daily Sprint', weekly: 'Weekly Sprint', monthly: 'Monthly', custom: 'Custom' };
  ...
  function renderHubShell(container) {
    ...
    container.innerHTML = `<div class="squads-page">
      <div class="squads-hero">
        ...
        <h1 class="squads-title">${subView === 'detail' ? 'Squad Leaderboard' : subView === 'results' ? 'Competition Results' : 'Compete with Friends'}</h1>
        <p class="squads-sub">${subView === 'results' ? 'Final standings, event stats, and performance analysis for this competition.' : subView ? 'Delta scoring — only progress during the competition window counts.' : 'Create or join private coding competitions. Your LeetCode username stays hidden from other members.'}</p>
      </div>
