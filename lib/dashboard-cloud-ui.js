...
(function () {
  ...
  function isCompletingAuthRedirect() {
    try {
      const fnPending = cloud()?.isPendingAuthRedirect;
      const fnCallback = cloud()?.isAuthCallbackUrl;
      return (typeof fnPending === 'function' && fnPending())
  ...
  function shouldNavigateAfterSignIn() {
  ...
  function firebaseAuthHelpHtml() {
    ...
    const domainHint = isWeb
      ...
      : (typeof chrome !== 'undefined' && chrome.runtime?.id
        ? escapeHtml(`chrome-extension://${chrome.runtime.id}`)
        : 'chrome-extension://YOUR_EXTENSION_ID');
    const redirectHint = typeof chrome !== 'undefined' && chrome.identity?.getRedirectURL
      ? escapeHtml(chrome.identity.getRedirectURL())
    ...
    if (isWeb) {
      ...
      <div id="authSetupPanel" class="auth-setup-panel">
        <p>Google Sign-In uses <strong>Firebase Authentication</strong> (popup / redirect).</p>
    ...
    return `
      <button type="button" id="authSetupToggle" class="auth-setup-toggle">⚙ Google sign-in setup — click to expand</button>
      <div id="authSetupPanel" class="auth-setup-panel">
        <p>Extension Google sign-in uses <strong>chrome.identity.launchWebAuthFlow</strong> — no remote scripts (MV3-safe).</p>
        <p class="mt-2">One-time setup:</p>
        <ul class="mt-2 space-y-1 text-[11px] list-disc ml-4">
          <li>Firebase Console → Authentication → Google → copy the <strong>Web client ID</strong> into <code>VITE_FIREBASE_GOOGLE_WEB_CLIENT_ID</code> in <code>.env</code>, then <code>npm run build</code></li>
          <li>Google Cloud Console → same OAuth client → add redirect URI: <code>${redirectHint}</code></li>
          <li>Firebase → Authorized domains → add <code>${domainHint}</code></li>
        </ul>
      </div>`;
  }
