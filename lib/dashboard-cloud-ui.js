...
(function () {
  ...
  function renderSignInPage(state) {
    ...
    if (!firebaseReady()) {
      container.innerHTML = `
        <div class="glass-panel p-8 rounded-xl space-y-4 text-center">
          <span class="material-symbols-outlined text-5xl text-on-surface-variant/40">cloud_off</span>
          <h2 class="text-lg font-semibold text-on-surface">Firebase not configured</h2>
          <p class="text-sm text-on-surface-variant">Add credentials to <code>.env</code> and run <code>npm run build</code>, then reload the extension.</p>
        </div>`;
      return;
    }

    if (state.loading) {
      container.innerHTML = `<div class="glass-panel p-8 rounded-xl text-center text-on-surface-variant">Restoring session…</div>`;
      return;
    }
    ...
    if (state.user) {
      ...
      container.innerHTML = `
        <div class="auth-card auth-signed-in">
          ${avatar}
          <h3>Welcome back!</h3>
          <p>${escapeHtml(state.user.displayName || state.user.email)}</p>
          <div class="auth-signed-in-actions">
            <button id="goToProfileBtn" class="auth-btn-outline">View Profile</button>
            <button id="goToOverviewBtn" class="auth-btn-primary" style="width:auto;padding:11px 24px;">Go to Dashboard</button>
    ...
    container.innerHTML = `
      <div class="auth-page">
        <div class="auth-hero">
          <div class="auth-hero-badge"><span class="material-symbols-outlined text-sm">cloud_sync</span> LeetLens Cloud</div>
          <h2>${isSignUp ? 'Start your coding journey' : 'Pick up where you left off'}</h2>
          <p>${isSignUp ? 'Create a free account to sync LeetCode progress, GitHub repos, and study plans across devices.' : 'Sign in to access your synced problems, analytics, and weekly plans.'}</p>
          <div class="auth-features">
            <div class="auth-feature"><span class="material-symbols-outlined">code</span> LeetCode profile sync</div>
            <div class="auth-feature"><span class="material-symbols-outlined">school</span> Striver A2Z progress</div>
            <div class="auth-feature"><span class="material-symbols-outlined">insights</span> Analytics & heatmaps</div>
          </div>
        </div>

        <div class="auth-card">
          <div class="auth-tabs">
            <button id="authTabSignIn" class="auth-tab ${!isSignUp ? 'active' : ''}">Sign In</button>
            <button id="authTabSignUp" class="auth-tab ${isSignUp ? 'active' : ''}">Sign Up</button>
          </div>
          ...
          <div class="auth-field">
            ...
            <input id="authDisplayName" type="text" placeholder="Optional" autocomplete="name" />
          </div>` : ''}
          <div class="auth-field">
          ...
          <button id="btnEmailAuth" class="auth-btn-primary">
          ...
          <button id="btnGoogleAuth" class="auth-btn-google" ${oauthReady() ? '' : 'disabled'}>
            ${GOOGLE_BTN_HTML}
            <span>${isSignUp ? 'Sign up with Google' : 'Sign in with Google'}</span>
          </button>

          ${state.error ? `<div class="auth-status error mt-3">${escapeHtml(state.error)}</div>` : ''}
          ${oauthSetupHtml()}
        </div>
      </div>`;
  ...
  function renderProfilePage(state) {
    ...
    if (!state.user) {
      container.innerHTML = `
        <div class="glass-panel p-8 rounded-xl text-center space-y-4">
          <span class="material-symbols-outlined text-5xl text-on-surface-variant/40">lock</span>
          <h2 class="text-lg font-semibold text-on-surface">Sign in required</h2>
          <p class="text-sm text-on-surface-variant">You need to sign in to view your profile.</p>
          <button id="profileGoSignIn" class="px-6 py-3 rounded-xl bg-primary text-black text-sm font-bold uppercase">Sign In</button>
        </div>`;
    ...
    container.innerHTML = `
      <div class="glass-panel p-6 rounded-xl space-y-6">
        <div class="flex items-center gap-4 pb-6 border-b border-outline-variant/10">
          ${state.user.photoURL
            ? `<img src="${escapeHtml(state.user.photoURL)}" class="w-16 h-16 rounded-full border-2 border-primary/30" alt="">`
            : `<div class="w-16 h-16 rounded-full bg-primary/20 flex items-center justify-center"><span class="material-symbols-outlined text-3xl text-primary">person</span></div>`}
          <div class="flex-1">
            <h2 class="text-lg font-bold text-on-surface">${escapeHtml(state.user.displayName || 'User')}</h2>
            <p class="text-sm text-on-surface-variant">${escapeHtml(state.user.email)}</p>
            <p class="text-xs text-on-surface-variant/70 mt-1">Member since ${escapeHtml(memberSince)}</p>
          </div>
          <button id="profileBtnSignOut" class="auth-btn-logout">Sign Out</button>
        </div>

        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div class="p-4 rounded-xl bg-surface-container/50"><div class="text-[10px] uppercase text-on-surface-variant">Total Solved</div><div class="text-xl font-bold text-primary mt-1">${stats.totalSolved ?? '—'}</div></div>
          <div class="p-4 rounded-xl bg-surface-container/50"><div class="text-[10px] uppercase text-on-surface-variant">Streak</div><div class="text-xl font-bold mt-1">🔥 ${stats.streak ?? '—'}</div></div>
          <div class="p-4 rounded-xl bg-surface-container/50"><div class="text-[10px] uppercase text-on-surface-variant">Easy</div><div class="text-xl font-bold text-diff-easy mt-1">${stats.easySolved ?? '—'}</div></div>
          <div class="p-4 rounded-xl bg-surface-container/50"><div class="text-[10px] uppercase text-on-surface-variant">Hard</div><div class="text-xl font-bold text-diff-hard mt-1">${stats.hardSolved ?? '—'}</div></div>
        </div>

        <div class="space-y-3">
          <h3 class="text-sm font-semibold text-on-surface">LeetCode Account</h3>
          <div class="flex flex-wrap gap-2">
            <input id="profileLeetcodeInput" type="text" placeholder="LeetCode username" value="${escapeHtml(profile.leetcodeUsername || '')}"
              class="flex-1 min-w-[180px] px-4 py-2.5 rounded-xl bg-surface-container border border-outline-variant/20 text-sm text-on-surface outline-none focus:border-primary/50" />
            <button id="profileBtnLink" class="px-4 py-2.5 rounded-xl bg-surface-container-high text-xs font-bold uppercase tracking-wider">Link</button>
            <button id="profileBtnSync" class="px-4 py-2.5 rounded-xl border border-primary/40 text-primary text-xs font-bold uppercase" ${state.syncing ? 'disabled' : ''}>
              ${state.syncing ? 'Syncing…' : 'Sync Profile'}
            </button>
          </div>
          <div id="profileLinkStatus" class="sync-status"></div>
        </div>

        <div class="space-y-3 pt-4 border-t border-outline-variant/10">
          <h3 class="text-sm font-semibold text-on-surface">GitHub Account</h3>
          <div class="flex flex-wrap gap-2">
            <input id="profileGhInput" type="text" placeholder="GitHub username" value="${escapeHtml(profile.githubUsername || '')}"
              class="flex-1 min-w-[180px] px-4 py-2.5 rounded-xl bg-surface-container border border-outline-variant/20 text-sm text-on-surface outline-none focus:border-primary/50" />
            <button id="profileGhBtnLink" class="px-4 py-2.5 rounded-xl bg-surface-container-high text-xs font-bold uppercase tracking-wider">Link</button>
            <button id="profileGhBtnSync" class="px-4 py-2.5 rounded-xl border border-primary/40 text-primary text-xs font-bold uppercase" ${state.syncing ? 'disabled' : ''}>
              ${state.syncing ? 'Syncing…' : 'Sync GitHub'}
            </button>
          </div>
          <div id="profileGhStatus" class="sync-status info">${profile.githubLastSyncedAt ? `Last synced: ${new Date(profile.githubLastSyncedAt).toLocaleString()}` : ''}</div>
          ${profile.githubStats?.publicRepos != null ? `<p class="text-xs text-on-surface-variant">${profile.githubStats.publicRepos} repos · ${profile.githubStats.totalStars || 0} stars</p>` : ''}
        </div>

        <div class="space-y-3 pt-4 border-t border-outline-variant/10">
          <h3 class="text-sm font-semibold text-on-surface">Email Reminders</h3>
          <label class="flex items-center gap-3 text-sm text-on-surface-variant cursor-pointer">
            <input type="checkbox" id="profileEmailToggle" ${profile.emailRemindersEnabled !== false ? 'checked' : ''} class="w-4 h-4" />
            Send daily reminder at 10:00 AM IST if I haven't solved a problem
          </label>
        </div>
      </div>`;
      ...
      <div class="sync-hub">
        <div class="sync-card">
          <div class="sync-card-header">
            <div class="sync-card-icon lc"><span class="material-symbols-outlined">code</span></div>
            <div>
